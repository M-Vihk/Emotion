...
module.bundle.HMR_BUNDLE_ID = "2904e5189e80d2a0";
"use strict";
...
},{}],"hXkWO":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "gerarRelatorioEmocional", ()=>gerarRelatorioEmocional);
var _supabaseService = require("../../service/SupabaseService");
async function gerarRelatorioEmocional() {
    try {
        const user = await (0, _supabaseService.SupabaseService).getUsuarioAutenticado();
        if (!user) throw new Error("Usu\xe1rio n\xe3o autenticado.");
        const registros = await (0, _supabaseService.SupabaseService).getUltimosRegistrosEmocionais(user.id);
        if (registros.length == 0) throw new Error("Nenhum registro emocional encontrado para gerar relat\xf3rio.");
        const soma = {
            alegria: 0,
            tristeza: 0,
            medo: 0,
        ...
        await (0, _supabaseService.SupabaseService).salvarRelatorio(user.id, medias);
        console.log("Relat\xf3rio emocional gerado com sucesso!");
        return medias;
    } catch (err) {
...
async function gerarRelatorioComSugestao() {
    try {
        ...
        let emocaoPrincipal = "";
        let maiorMedia = 0;
        for(const emocao in medias)if (medias[emocao] > maiorMedia) {
            maiorMedia = medias[emocao];
            emocaoPrincipal = emocao;
        }
        const sugestoes = await (0, _supabaseService.SupabaseService).getSugestaoPorEmocao(emocaoPrincipal);
        const sugestaoAleatoria = sugestoes.length > 0 ? sugestoes[Math.floor(Math.random() * sugestoes.length)] : null;
        const listaMedias = document.getElementById("lista-medias");
        ...
        console.error("Erro ao gerar relat\xf3rio:", err.message);
    }
}
