...
module.bundle.HMR_BUNDLE_ID = "3ee790319917a0da";
"use strict";
...
class SupabaseService {
    ...
    static async verificarRegistroHoje(userId) {
        const hoje = new Date().toISOString().split("T")[0];
        const { data, error } = await (0, _supabaseClientJs.supabase).from("emocoes").select("id").eq("id_usuario", userId).eq("data_registro", hoje);
        if (error) throw new Error("Erro ao verificar registros: " + error.message);
        return data && data.length > 0;
    }
    static async buscarDadosUsuario(userId) {
        const { data: dadosUsuario, error: errorUsuario } = await (0, _supabaseClientJs.supabase).from("usuarios").select("nome, nascimento, genero").eq("id", userId).single();
        if (errorUsuario) throw new Error("Erro ao buscar dados do usu\xe1rio: " + errorUsuario.message);
        const { data: { user }, error: errorAuth } = await (0, _supabaseClientJs.supabase).auth.getUser();
        if (errorAuth || !user) throw new Error("Erro ao obter email do usu\xe1rio autenticado.");
        return {
    ...
    static async getUsuarioAtual() {
        const { data: { user }, error } = await (0, _supabaseClientJs.supabase).auth.getUser();
        if (error || !user) throw new Error("Usu\xe1rio n\xe3o autenticado.");
        return user;
    }
    static async sair() {
        try {
            ...
            console.log("Usu\xe1rio desconectado com sucesso!");
        } catch (err) {
            throw new Error(err.message || "Erro inesperado ao sair.");
        }
    }
    static async salvarDiario(titulo, data, pensamentos) {
        try {
            const { data: { user }, error: userError } = await (0, _supabaseClientJs.supabase).auth.getUser();
            if (userError || !user) throw new Error("Usu\xe1rio n\xe3o autenticado.");
            const { error } = await (0, _supabaseClientJs.supabase).from("diario").insert([
                {
                    id_usuario: user.id,
                    titulo: titulo,
                    data: data,
            ...
            if (error) throw new Error("Erro ao salvar o di\xe1rio: " + error.message);
            console.log("Entrada do di\xe1rio salva com sucesso!");
        } catch (err) {
            throw new Error(err.message || "Erro inesperado ao salvar o di\xe1rio.");
        }
    }
    static async listarDiarios() {
        try {
            const { data: { user }, error: userError } = await (0, _supabaseClientJs.supabase).auth.getUser();
            if (userError || !user) throw new Error("Usu\xe1rio n\xe3o autenticado.");
            const { data, error } = await (0, _supabaseClientJs.supabase).from("diario").select("*").eq("id_usuario", user.id).order("data", {
                ascending: false
            });
            if (error) throw new Error("Erro ao buscar o di\xe1rio: " + error.message);
            return data || [];
        } catch (err) {
            throw new Error(err.message || "Erro inesperado ao listar o di\xe1rio.");
        }
    }
    static async salvarRelatorio(userId, medias) {
        const { error } = await (0, _supabaseClientJs.supabase).from("relatorios").insert([
            {
                id_usuario: userId,
        ...
        if (error) throw new Error("Erro ao salvar relat\xf3rio: " + error.message);
    }
    static async getUltimosRegistrosEmocionais(userId, limite = 7) {
        const { data, error } = await (0, _supabaseClientJs.supabase).from("emocoes").select("alegria, tristeza, medo, ansiedade, raiva").eq("id_usuario", userId).order("data_registro", {
            ascending: false
        }).limit(limite);
    ...
    static async getSugestaoPorEmocao(emocao) {
        const { data, error } = await (0, _supabaseClientJs.supabase).from("sugestoes").select("id, sugestao_alegria, sugestao_tristeza, sugestao_medo, sugestao_ansiedade, sugestao_raiva");
        if (error) throw new Error("Erro ao buscar sugest\xe3o: " + error.message);
        var lista = [];
        for(var i = 0; i < data.length; i++)lista.push(data[i]["sugestao_" + emocao]);
        return lista;
    }
}
...
var process = require("ffcd38581d30f432");
...
if (shouldShowDeprecationWarning()) console.warn(`\u{26A0}\u{FE0F}  Node.js 18 and below are deprecated and will no longer be supported in future versions of @supabase/supabase-js. ` + `Please upgrade to Node.js 20 or later. ` + `For more information, visit: https://github.com/orgs/supabase/discussions/37217`);

},{"ffcd38581d30f432":"euskh","./SupabaseClient":"3Lzwe","@supabase/auth-js":"1NgD7","@supabase/postgrest-js":"h64eV","@supabase/functions-js":false,"@supabase/realtime-js":"bpWhq","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"euskh":[function(require,module,exports,__globalThis) {
...
},{}],"3Lzwe":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _functionsJs = require("@supabase/functions-js");
var _postgrestJs = require("@supabase/postgrest-js");
     ...
     */ uploadOrUpdate(method, path, fileBody, fileOptions) {
        return __awaiter(this, void 0, void 0, function*() {
            try {
                ...
                const data = yield (method == 'PUT' ? (0, _fetch.put) : (0, _fetch.post))(this.fetch, `${this.url}/object/${_path}`, body, Object.assign({
                    headers
...
const handleError = (error, reject, options)=>__awaiter(void 0, void 0, void 0, function*() {
        ...
        if (error instanceof Res && !(options === null || options === void 0 ? void 0 : options.noResolveJson)) error.json().then((err)=>{
            ...
            reject(new (0, _errors.StorageApiError)(_getErrorMessage(err), status, statusCode));
        }).catch((err)=>{
...
class StorageBucketApi {
    constructor(url, headers = {}, fetch, opts){
        ...
        if (opts === null || opts === void 0 ? void 0 : opts.useNewHostname) {
            ...
            if (isSupabaseHost && !baseUrl.hostname.includes('storage.supabase.')) baseUrl.hostname = baseUrl.hostname.replace('supabase.', 'storage.supabase.');
        }
